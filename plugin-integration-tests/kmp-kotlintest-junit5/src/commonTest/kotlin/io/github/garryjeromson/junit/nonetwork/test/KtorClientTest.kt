 ...
 * with both @BlockNetworkRequests and @AllowNetworkRequests annotations in KMP context (kotlin.test + JUnit 5).
 *
...
class KtorClientTest {
    ...
    @Test
    @BlockNetworkRequests
    fun `ktor client with spaces in test name is blocked in kotlin test junit5`() {
        val exception = assertFailsWith<Exception> {
            makeKtorRequest()
        }
        // Ktor/OkHttp wraps NetworkRequestAttemptedException in IOException on Android
