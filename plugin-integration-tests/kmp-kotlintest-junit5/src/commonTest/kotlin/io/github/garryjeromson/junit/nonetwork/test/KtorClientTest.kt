 ...
 * with both @NoNetworkTest and @AllowNetwork annotations in KMP context (kotlin.test + JUnit 5).
 *
...
class KtorClientTest {
    ...
    @Test
    @NoNetworkTest
    fun `ktor client with spaces in test name should be blocked in kotlin test junit5`() {
        val exception = assertFailsWith<Exception> {
            makeKtorRequest()
        }
        // Ktor/OkHttp wraps NetworkRequestAttemptedException in IOException on Android
