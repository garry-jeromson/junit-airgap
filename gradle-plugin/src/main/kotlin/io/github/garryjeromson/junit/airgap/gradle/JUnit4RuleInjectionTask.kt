 ...
 * - Injects a public Rule field: `@Rule public AirgapRule noNetworkRule = new AirgapRule()`
...
abstract class JUnit4RuleInjectionTask : DefaultTask() {
     ...
     *
     * Note: This implementation uses ByteBuddy's type pool to reference AirgapRule by name,
    ...
    private fun injectRule(
        ...
        // Reference the AirgapRule class by name using ByteBuddy's type pool
        // This doesn't require the class to be in the gradle plugin's classpath
        val ruleClassName = "io.github.garryjeromson.junit.airgap.AirgapRule"
        ...
        // For each constructor: call super(), then initialize noNetworkRule = new AirgapRule()
