...
class JUnit4RuleInjectionTest {
    ...
    class HasRuleTest {
        @org.junit.Rule
        val noNetworkRule = Any() // Using Any as placeholder since AirgapRule may not be available
    ...
    @Test
    fun `hasAirgapRule returns false when field does not exist`() {
        val result = hasAirgapRule(SimpleJUnit4Test::class.java)
        assertFalse(result, "Should return false when noNetworkRule field does not exist")
    }

    @Test
    fun `hasAirgapRule returns true when field exists`() {
        val result = hasAirgapRule(HasRuleTest::class.java)
        assertTrue(result, "Should return true when noNetworkRule field exists")
    }

    @Test
    fun `hasAirgapRule detects field in parent class`() {
        val result = hasAirgapRule(ChildOfParentWithRule::class.java)
    ...
    fun `injectRule adds noNetworkRule field to class`() {
        ...
        // Create a mock AirgapRule class for testing
        val mockRuleClass = createMockRuleClass()
        saveClassToFile(mockRuleClass, "MockAirgapRule")
    ...
    /**
     * Standalone implementation of hasAirgapRule for testing
     */
    private fun hasAirgapRule(clazz: Class<*>): Boolean {
        return try {
    ...
    private fun injectRule(
        ...
        // For testing, use a mock rule class instead of the real AirgapRule
        val ruleClass = classLoader.loadClass("MockAirgapRule")
